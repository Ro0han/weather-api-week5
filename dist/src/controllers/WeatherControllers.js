 ...
 */
export const generatedWeatherData = async (req, res) => {
    // Check if there are any validation errors
    const errors = validationResult(req);
    //we will log them and send a 400 status code
    if (!errors.isEmpty()) {
        console.error("Validtion error", errors.mapped());
        res.status(400).json({ errors: errors.array() });
        return;
    }
    //We will use a try catch block to catch any errrors
    try {
        //Get the city param from the request
        const { city } = req.params;
        console.log(city);
        //We will create a variable with the type of WeatherData
...
//# sourceMappingURL=WeatherControllers.js.map